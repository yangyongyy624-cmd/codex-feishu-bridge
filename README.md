...
手机飞书 / Codex APP 远程 Mac 执行命令、运行代码、控制应用。

**两套链路共用同一个本地代理（`127.0.0.1:4000`），同时运行，不冲突。**
...
| **ToggleFeishu** | Mac 菜单栏一键开关飞书连接，保护隐私/专注工作 |
...
Mac 菜单栏一键切换飞书连接状态。双击 `ToggleFeishu.app` → 输入密码 → 完成。

原理：通过 `/etc/hosts` 将飞书 WebSocket 域名指向 `127.0.0.1`，断开长连接。安全、可逆、Web UI 不受影响。

完整安装见 [docs/SKILL.md](docs/SKILL.md)。
